...
const postCartIdController = async(req,res) => {
    try{
        ...
        res.status(400).send({error: "400", message: `Error agregando p`});
    }
}
...
const getPurchaseController = async(req,res) => {
    try{
        ...
        if(prodsInCart.products.length === 0){
            ...
            throw new Error(`No existen productos en el carrito`)
        }
        let sumProds = 0
        let notInStock = [];
        ...
        console.error(`Error processing request: ${error}`)
        res.status(500).send({ error: "500", message: `Error realizando la compra del carrito. ${error}` })
    }
}
