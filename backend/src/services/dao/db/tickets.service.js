    ...
    createTicket = async (sumCart, userEmail) => {
        try{
            ...
            const purchaseMade = {
                    ...
                    to: userEmail,
                    subject: 'Su compra ha sido realizada correctamente',
                    html: `<div><h2> Agradecemos su compra. Su ticket: ${result.code} </h2>
                    <h3>Fecha: ${result.purchase_datetime} </h3>
                    <h3>Monto: ${result.amount} </h3>

                    </div>`
                }
            await emailService.sendEmail(purchaseMade)
